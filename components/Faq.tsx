...
export default function Faq() {
  ...
  return (
    <section
      id="faq"
      className="py-24 md:py-32 bg-gradient-to-b from-[#fbfaff] to-white text-slate-800 relative z-20 overflow-hidden border-t border-purple-100/40 select-none text-left font-sans"
    >
      ...
      <div className="max-w-3xl mx-auto px-6 relative z-10 space-y-16">
        
        {/* Title Block */}
        <div className="text-center space-y-4 max-w-xl mx-auto">
          <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-purple-50 border border-purple-100/80 text-[10px] font-bold uppercase tracking-widest text-[#7c3aed]">
            <HelpCircle className="w-3.5 h-3.5 text-[#7c3aed]" />
            <span>Faq Center</span>
          </div>
          
          <h2 className="text-[34px] sm:text-[40px] font-black text-slate-900 tracking-tight leading-tight">
        ...
        {/* Minimalist Accordion List with line separators (No blocky boxes) */}
        <div className="divide-y divide-purple-100/70 border-t border-b divide-solid border-purple-100/70">
          {FAQS.map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between text-left cursor-pointer select-none focus:outline-none group py-1"
                >
                  <span className={`text-[15px] sm:text-[16px] font-bold leading-snug pr-8 transition-colors duration-200 ${
                  ...
                  <div className="relative w-5 h-5 flex items-center justify-center shrink-0">
                    <div className={`absolute w-4 h-[2px] rounded-full transition-transform duration-300 bg-[#7c3aed] ${isOpen ? "rotate-90" : ""}`} />
                    <div className="absolute w-[2px] h-4 rounded-full transition-transform duration-300 bg-[#7c3aed]" style={{ transform: isOpen ? "rotate(90deg) scale(0)" : "none" }} />
                  </div>
                </button>
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="text-[13px] sm:text-[13.5px] text-slate-500 leading-relaxed font-semibold pt-3 pb-2 pr-6">
                        {item.a}
                      </div>
                    </motion.div>
                  )}
