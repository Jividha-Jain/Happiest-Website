...
import { CheckCircle2, Mail, ArrowRight, Users, TrendingUp, Heart } from "lucide-react";
...
export default function Cta({ scrollTo }: CtaProps) {
  ...
  return (
    <section
      id="cta"
      className="relative w-full min-h-[720px] sm:min-h-[860px] lg:min-h-[920px] flex items-center justify-center select-none py-16 sm:py-28 lg:py-36 overflow-hidden bg-[#04040c]"
    >
      {/* Deep radial bg */}
      ...
      {/* Concentric rings - (Desktop/Tablet: sm:block) */}
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200px] h-[200px] sm:w-[240px] sm:h-[240px] bg-indigo-400/25 rounded-full blur-[60px] sm:blur-[70px] pointer-events-none z-0" />

      {/* Orbiting creator cards (DESKTOP / TABLET ONLY: hidden sm:block - 100% UNTOUCHED) */}
      <div className="hidden sm:block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 sm:w-[800px] sm:h-[800px] lg:w-[1080px] lg:h-[1080px] rounded-full pointer-events-none z-10">
        {creatorCards.map((creator, idx) => {
          ...
          const radius = 37.5; // Positions all cards completely inside outer circle on desktop
      ...
      <div className="relative z-20 flex flex-col items-center justify-center text-center px-4 sm:px-6 max-w-sm sm:max-w-3xl mx-auto gap-4 sm:gap-6">

        {/* Top Badge */}
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-purple-500/15 border border-purple-400/25 text-purple-300 text-[10px] sm:text-[11px] font-bold tracking-wider uppercase backdrop-blur-md">
        ...
        <div className="space-y-1">
          <h2 className="text-[30px] sm:text-5xl lg:text-[56px] font-black tracking-tight leading-[1.12] font-display text-white">
            Build your own community.
          </h2>
          <div className="relative inline-block">
            <h2 className="text-[30px] sm:text-5xl lg:text-[56px] font-black tracking-tight leading-[1.12] font-display bg-gradient-to-r from-[#d8b4fe] via-[#c084fc] to-[#a855f7] bg-clip-text text-transparent">
              Grow your business.
            </h2>
            {/* Underline swoosh */}
            <svg
              className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-44 sm:w-60 h-2.5 text-[#a855f7]/70 pointer-events-none"
              viewBox="0 0 200 12"
              fill="none"
            >
        ...
        {/* Subtext */}
        <p className="text-slate-300/90 text-xs sm:text-base font-normal leading-relaxed max-w-md pt-1">
          Join thousands of creators and businesses already growing with Happiest Team.
        </p>

        {/* Creator Avatars Row (Uses the same images from creatorCards) */}
          ...
          <div className="relative flex items-center justify-center">
            <div className="flex items-center -space-x-2.5">
              {creatorCards.slice(0, 4).map((c, i) => (
                <div
                  key={i}
                  className="w-10 h-10 sm:w-11 sm:h-11 rounded-full overflow-hidden border-2 border-[#04040c] shadow-md relative bg-slate-800"
                >
                  <Image src={c.img} alt={c.name} fill className="object-cover" unoptimized />
                </div>
              ))}
              {/* +10K Bubble */}
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-[#4a1c8c] text-white font-black text-[11px] sm:text-xs flex items-center justify-center border-2 border-[#04040c] shadow-md relative shrink-0">
                +10K
              </div>
            </div>
        ...
        {/* Form Box Card */}
        <div className="w-full max-w-[360px] sm:max-w-md bg-white rounded-[26px] p-2.5 sm:p-2 shadow-[0_20px_50px_rgba(0,0,0,0.7),0_0_35px_rgba(124,92,255,0.2)] border border-white/20 mt-1">
          <form onSubmit={handleSubmit} className="flex flex-col gap-2">
            ...
            <div className="flex items-center bg-[#f7f5fd] border border-purple-100/90 rounded-2xl px-3 py-2 text-left">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 font-medium outline-none"
              />
            </div>
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-[#4C1D95] via-[#5B21B6] to-[#6D28D9] hover:from-[#3B137E] hover:to-[#5B21B6] text-white font-extrabold text-xs sm:text-sm py-3 sm:py-3.5 rounded-2xl transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 shadow-[0_4px_18px_rgba(91,33,182,0.4)] hover:scale-[1.01]"
            >
          ...
          {submitted && (
            <div className="mt-2.5 flex items-center justify-center gap-1.5 text-xs text-emerald-600 font-semibold py-1">
